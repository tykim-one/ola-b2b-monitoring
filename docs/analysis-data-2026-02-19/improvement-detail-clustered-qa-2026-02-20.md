# Q&A 페어 답변유형 클러스터링 리포트 (A/B/C/D 유지)

> 작성일: 2026-02-20
>
> 기준: 기존 A/B/C/D 원본 리스트를 그대로 사용하고, `user_input`-`llm_response` 페어 중심으로 답변 유형을 클러스터링
>
> 포함 필드: `session_id`, `x_enc_data`, `response_node`, `category`, `question_type`, `user_input`, `llm_response_1000`

---

## 분류 규칙 요약

- **정책/투자자문 제한 응답**: 법률/내부규정/투자자문 제한 문구 포함
- **질문 구체화 요청**: "질문 범위가 넓다", "구체적으로 질문" 패턴
- **입력 이해 실패 안내**: "정확히 이해하지 못"/"다시 질문" 패턴
- **지원불가 템플릿 응답**: 금융 챗봇 고정 안내 + 답변 어려움 템플릿
- **데이터 부재 단순 안내**: 데이터 없음 안내 중심, 내용 확장 적음
- **부분답변 + 한계고지**: 수치/표/서술 후 데이터 한계 문구 동시 포함
- **후보 나열형 모호성 응답**: 여러 후보 종목/티커 나열로 답변
- **모호성 해소 유도**: 모호 질문에 대해 추가 맥락 요구/제시
- **구조화/수치 요약형**: 표/헤더/수치 기반의 정형 답변
- **기타 응답형**: 상기 규칙에 명확히 해당하지 않는 케이스
...
- 총 건수: **189건**
- 원본: `detail_2_2_nonfinal_rows.csv`

### 클러스터 요약

| 답변유형 | 내부코드 | 건수 | 비율(%) |
|---|---|---:|---:|
| 모호성 해소 유도 | `ambiguous_resolution_prompt` | 44 | 23.28 |
| 지원불가 템플릿 응답 | `unsupported_refusal_template` | 33 | 17.46 |
| 정책/투자자문 제한 응답 | `policy_safety_refusal` | 26 | 13.76 |
| 후보 나열형 모호성 응답 | `ambiguous_candidate_list` | 22 | 11.64 |
| 질문 구체화 요청 | `clarification_prompt` | 15 | 7.94 |
| 입력 이해 실패 안내 | `input_not_understood` | 15 | 7.94 |
| 기타 응답형 | `other_response_type` | 13 | 6.88 |
| 부분답변 + 한계고지 | `partial_answer_with_limitations` | 11 | 5.82 |
| 데이터 부재 단순 안내 | `data_gap_notice` | 10 | 5.29 |

### 클러스터별 상세 리스트

#### 모호성 해소 유도 (`ambiguous_resolution_prompt`) - 44건
...
#### 지원불가 템플릿 응답 (`unsupported_refusal_template`) - 33건
...
#### 정책/투자자문 제한 응답 (`policy_safety_refusal`) - 26건
...
#### 후보 나열형 모호성 응답 (`ambiguous_candidate_list`) - 22건
...
#### 질문 구체화 요청 (`clarification_prompt`) - 15건
...
#### 입력 이해 실패 안내 (`input_not_understood`) - 15건
...
#### 기타 응답형 (`other_response_type`) - 13건
...
#### 부분답변 + 한계고지 (`partial_answer_with_limitations`) - 11건
...
#### 데이터 부재 단순 안내 (`data_gap_notice`) - 10건
...
- 총 건수: **34건**
- 원본: `detail_12_4_high_risk_rows.csv`

### 클러스터 요약

| 답변유형 | 내부코드 | 건수 | 비율(%) |
|---|---|---:|---:|
| 부분답변 + 한계고지 | `partial_answer_with_limitations` | 20 | 58.82 |
| 데이터 부재 단순 안내 | `data_gap_notice` | 14 | 41.18 |

### 클러스터별 상세 리스트

#### 부분답변 + 한계고지 (`partial_answer_with_limitations`) - 20건
...
#### 데이터 부재 단순 안내 (`data_gap_notice`) - 14건
...
- 총 건수: **112건**
- 원본: `detail_12_5_data_gap_rows.csv`

### 클러스터 요약

| 답변유형 | 내부코드 | 건수 | 비율(%) |
|---|---|---:|---:|
| 부분답변 + 한계고지 | `partial_answer_with_limitations` | 77 | 68.75 |
| 후보 나열형 모호성 응답 | `ambiguous_candidate_list` | 18 | 16.07 |
| 데이터 부재 단순 안내 | `data_gap_notice` | 15 | 13.39 |
| 구조화/수치 요약형 | `structured_or_quantitative_answer` | 2 | 1.79 |

### 클러스터별 상세 리스트

#### 부분답변 + 한계고지 (`partial_answer_with_limitations`) - 77건
...
#### 후보 나열형 모호성 응답 (`ambiguous_candidate_list`) - 18건
...
#### 데이터 부재 단순 안내 (`data_gap_notice`) - 15건
...
#### 구조화/수치 요약형 (`structured_or_quantitative_answer`) - 2건
...
- 총 건수: **78건**
- 원본: `detail_mixed_session_nonfinal_rows.csv`

### 클러스터 요약

| 답변유형 | 내부코드 | 건수 | 비율(%) |
|---|---|---:|---:|
| 모호성 해소 유도 | `ambiguous_resolution_prompt` | 22 | 28.21 |
| 지원불가 템플릿 응답 | `unsupported_refusal_template` | 13 | 16.67 |
| 정책/투자자문 제한 응답 | `policy_safety_refusal` | 11 | 14.10 |
| 질문 구체화 요청 | `clarification_prompt` | 10 | 12.82 |
| 후보 나열형 모호성 응답 | `ambiguous_candidate_list` | 10 | 12.82 |
| 기타 응답형 | `other_response_type` | 5 | 6.41 |
| 데이터 부재 단순 안내 | `data_gap_notice` | 3 | 3.85 |
| 입력 이해 실패 안내 | `input_not_understood` | 2 | 2.56 |
| 부분답변 + 한계고지 | `partial_answer_with_limitations` | 2 | 2.56 |

### 클러스터별 상세 리스트

#### 모호성 해소 유도 (`ambiguous_resolution_prompt`) - 22건
...
#### 지원불가 템플릿 응답 (`unsupported_refusal_template`) - 13건
...
#### 정책/투자자문 제한 응답 (`policy_safety_refusal`) - 11건
...
#### 질문 구체화 요청 (`clarification_prompt`) - 10건
...
#### 후보 나열형 모호성 응답 (`ambiguous_candidate_list`) - 10건
...
#### 데이터 부재 단순 안내 (`data_gap_notice`) - 3건
...
#### 입력 이해 실패 안내 (`input_not_understood`) - 2건
...
#### 부분답변 + 한계고지 (`partial_answer_with_limitations`) - 2건
