...
export default function FAQAnalysisSection() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const request: FAQAnalysisRequest = {
        periodDays,
        topN,
        ...(tenantId && { tenantId }),
      };
      ...
      setError(err instanceof Error ? err.message : 'FAQ 분석 중 오류가 발생했습니다.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="bg-slate-800 rounded-lg border border-slate-700">
      ...
      <div className="p-4 border-b border-slate-700">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-white">FAQ 분석 (자주 묻는 질문)</h3>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm text-slate-400">기간:</label>
            <select
              value={periodDays}
          ...
          {/* Top N Select */}
          <div className="flex items-center gap-2">
            <label className="text-sm text-slate-400">Top:</label>
            <select
              value={topN}
          ...
          {/* Tenant Select */}
          <div className="flex items-center gap-2">
            <label className="text-sm text-slate-400">테넌트:</label>
            <select
              value={tenantId}
              onChange={(e) => setTenantId(e.target.value)}
              className="bg-slate-900 border border-slate-600 rounded px-3 py-1.5 text-sm text-white focus:outline-none focus:border-blue-500 min-w-[120px]"
              disabled={isLoading}
            >
          ...
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-600 disabled:cursor-not-allowed text-white px-4 py-1.5 rounded text-sm font-medium transition-colors"
          >
            {isLoading ? (
              <>
      ...
      <div className="p-4">
        {/* Error State */}
        {error && (
          <div className="flex items-center gap-2 bg-rose-900/20 border border-rose-800 rounded-lg p-4 text-rose-400">
        ...
        {isLoading && (
          <div className="text-center py-12">
            ...
            <p className="text-slate-400">FAQ를 분석하고 있습니다...</p>
            <p className="text-slate-500 text-sm mt-1">LLM 클러스터링 및 사유 분석 진행 중</p>
          </div>
        )}
