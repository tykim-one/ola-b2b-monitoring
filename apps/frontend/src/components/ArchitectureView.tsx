...
const ArchitectureView: React.FC = () => {
  return (
    <div className="p-8 h-full overflow-y-auto">
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-white mb-2">System Architecture Design</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-12 relative">
        {/* Source */}
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 relative group hover:border-blue-500 transition-colors">
          <div className="absolute -right-4 top-1/2 -translate-y-1/2 hidden lg:block text-slate-600">
          ...
          <div className="text-xl font-semibold text-white mb-2">B2B Partners</div>
          <p className="text-sm text-slate-400">External services sending webhooks, API calls, and transaction logs.</p>
        </div>

        {/* Storage */}
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 relative group hover:border-yellow-500 transition-colors">
          <div className="absolute -right-4 top-1/2 -translate-y-1/2 hidden lg:block text-slate-600">
             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14m-7-7l7 7-7 7"/></svg>
          </div>
          <h3 className="text-yellow-400 font-bold mb-2 uppercase text-xs tracking-wider">Data Warehouse</h3>
          <div className="text-xl font-semibold text-white mb-2">Google BigQuery</div>
          <p className="text-sm text-slate-400">Massive scale storage. Logs are partitioned by date and clustered by PartnerID.</p>
        </div>

        {/* Backend */}
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 relative group hover:border-red-500 transition-colors">
          <div className="absolute -right-4 top-1/2 -translate-y-1/2 hidden lg:block text-slate-600">
             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14m-7-7l7 7-7 7"/></svg>
          </div>
          <h3 className="text-red-400 font-bold mb-2 uppercase text-xs tracking-wider">Backend API</h3>
          <div className="text-xl font-semibold text-white mb-2">NestJS</div>
         ...
         {/* Client */}
         <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 lg:col-span-2 relative group hover:border-emerald-500 transition-colors">
          <h3 className="text-emerald-400 font-bold mb-2 uppercase text-xs tracking-wider">Frontend Application</h3>
          <div className="text-xl font-semibold text-white mb-2">Next.js (React)</div>
          <p className="text-sm text-slate-400">
      ...
      <div className="space-y-6">
        <h3 className="text-2xl font-bold text-white">Implementation Strategy</h3>
